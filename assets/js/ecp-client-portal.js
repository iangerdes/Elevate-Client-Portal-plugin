 ...
 * @comment Added an AJAX handler for the final password reset submission form.
 */
...
jQuery(function ($) {
    ...
    if (!portalWrapper.length && !$('body').find('.ecp-login-form-wrapper').length) return;
    
    const ZIP_SIZE_LIMIT_BYTES = 250 * 1024 * 1024; 
    ...
    function updateSelectionInfo() {
        let totalBytes = 0;
        const checkedFiles = $('.ecp-file-checkbox:checked:not(:disabled)');
        const selectionInfo = $('#ecp-selection-info');
        const zipButton = $('#ecp-create-zip-btn');
        ...
        if (checkedFiles.length > 0) {
            selectionInfo.text(`Selected ${checkedFiles.length} file(s) (${formatBytes(totalBytes)})`);
            if (totalBytes > ZIP_SIZE_LIMIT_BYTES) {
                selectionInfo.addClass('warning').text(selectionInfo.text() + ` - Limit of ${formatBytes(ZIP_SIZE_LIMIT_BYTES)} exceeded.`);
    ...
    $('body').on('submit', '#ecp-ajax-login-form', function(e) {
        e.preventDefault();
        const form = $(this);
        const btn = form.find('#ecp-login-submit-btn');
        const errorContainer = $('#ecp-login-error-container');
        
        btn.prop('disabled', true).text('Logging In...');
        errorContainer.slideUp().html('');

        $.post(ecp_ajax.ajax_url, {
            action: 'ecp_ajax_login',
            nonce: ecp_ajax.nonces.ajaxLoginNonce,
        ...
        .fail(() => {
            errorContainer.html('An unknown server error occurred. Please try again.').slideDown();
            btn.prop('disabled', false).text('Log In');
        });
    });

    $('body').on('submit', '#ecp-ajax-lost-password-form', function(e) {
        e.preventDefault();
        const form = $(this);
        const btn = form.find('button[type="submit"]');
        const msgContainer = $('#ecp-login-messages');

        btn.prop('disabled', true).text('Sending...');
        msgContainer.html('').hide();

        $.post(ecp_ajax.ajax_url, {
            action: 'ecp_lost_password',
            nonce: ecp_ajax.nonces.lostPasswordNonce,
            user_login: form.find('#user_login').val()
        })
        .done(response => {
            const noticeClass = response.success ? 'ecp-login-message' : 'ecp-login-error';
            msgContainer.html(`<p class="${noticeClass}">${response.data.message}</p>`).slideDown();
            if (response.success) {
                form.find('input[type="text"]').val('');
            }
        })
        .fail(() => {
            msgContainer.html('<p class="ecp-login-error">An unknown server error occurred.</p>').slideDown();
        })
        .always(() => {
    ...
    $('body').on('submit', '#ecp-ajax-reset-password-form', function(e) {
        e.preventDefault();
        const form = $(this);
        const btn = form.find('button[type="submit"]');
        const msgContainer = $('#ecp-reset-password-messages');

        btn.prop('disabled', true).text('Resetting...');
        msgContainer.html('').hide();

        $.post(ecp_ajax.ajax_url, {
            action: 'ecp_reset_password',
            nonce: ecp_ajax.nonces.resetPasswordNonce,
        ...
        .done(response => {
            ...
            const successMessage = response.data.message + ` Please <a href="${loginUrl}">log in</a> with your new password.`;
            msgContainer.html(`<p class="${noticeClass}">${response.success ? successMessage : response.data.message}</p>`).slideDown();
            if (response.success) {
                form.slideUp();
            }
        })
