 ...
 * @comment Added logic to handle the graceful logout redirect after an admin changes their own password.
 */
...
jQuery(function($) {
    ...
    function loadView(viewHtml) {
        ...
        $('#ecp-dashboard-main-content').find('#ecp_user_role').trigger('change');
    }
    ...
    adminDashboard.on('submit', '[id^="ecp-message-client-form-"]', function(e) {
        e.preventDefault();
        const form = $(this);
        ...
        $.post(ecp_ajax.ajax_url, form.serialize() + '&nonce=' + ecp_ajax.nonces.dashboardNonce)
            ...
            .fail(() => ECP_Admin.showAdminMessage('An unknown error occurred while sending the message.', 'error'))
        ...
        $.post(ecp_ajax.ajax_url, form.serialize() + '&nonce=' + ecp_ajax.nonces.dashboardNonce)
            .done(response => {
                ...
                if (response.success && form.attr('id') === 'ecp-user-details-form') {
                    // ** FIX: Handle the special case for self-password change **
                    if (response.data.self_password_change) {
                        ECP_Admin.showBlockingLoader('Redirecting to login...');
                        setTimeout(() => {
                            window.location.href = ecp_ajax.login_url;
                        }, 4000);
                    } else {
                        loadNamedView('user_list');
                    }
