 ...
 * @version 76.0.0 (Email Customization)
 * @comment Added filters to customize the new user notification email, setting a noreply@ address and directing password resets to the custom /login page.
 */
...
class ECP_Auth_Handler {
    ...
    private function __construct() {
        ...
        // ** NEW: Filters for customizing plugin emails. **
...
// ... existing code ...
    ...
    /**
     * Sets the 'From' address for all outgoing emails to noreply@.
     */
    ...
    /**
     * Sets the 'From' name for all outgoing emails to the site's name.
     */
    ...
    /**
     * Customizes the password reset email to point to the custom /login page.
     */
    public function custom_password_reset_message( $message, $key, $user_login, $user_data ) {
        $login_page = get_page_by_path( 'login' );
        if ( ! $login_page ) {
            return $message; // Fallback to default if login page doesn't exist
        }
        ...
        // Replace the URL within the angle brackets in the original message.
