 ...
 * @comment Updated the submenu page registration to work with the new, more reliable parent menu created by `add_menu_page`.
 */
...
class ECP_Settings {
    ...
    public function add_settings_page() {
        // The first submenu item's slug must match the parent's slug to become the default page.
        add_submenu_page('ecp-settings', __('Settings', 'ecp'), __('Settings', 'ecp'), 'manage_options', 'ecp-settings', [ $this, 'render_settings_page' ]);
        add_submenu_page('ecp-settings', __('S3 File Browser', 'ecp'), __('S3 File Browser', 'ecp'), 'manage_options', 'ecp-s3-browser', [ $this, 'render_s3_browser_page' ]);
        add_submenu_page('ecp-settings', __('Audit Log', 'ecp'), __('Audit Log', 'ecp'), 'manage_options', 'ecp-audit-log', [ $this, 'render_audit_log_page' ]);
    }
    ...
    public function register_settings() {
        register_setting( 'ecp_settings_group', 'ecp_style_options', [ $this, 'sanitize_style_options' ] );
        register_setting( 'ecp_settings_group', 'ecp_s3_options', [ $this, 'sanitize_s3_options' ] );
    ...
    /**
     * Sanitize the S3 options.
     */
    public function sanitize_s3_options( $input ) {
        $new_input = [];
        $fields = ['s3_bucket', 's3_region', 's3_subfolder', 's3_access_key', 's3_secret_key'];
