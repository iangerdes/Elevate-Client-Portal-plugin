 ...
 * @version 127.7.0 (AJAX Password Logout Fix)
 * @comment Implemented a graceful logout redirect when an admin changes their own password via AJAX to prevent 403 errors.
 */
...
class ECP_User_Manager {
    ...
    public static function get_client_users( $search_term = '' ) {
        ...
        if ( is_object($current_user) && in_array('administrator', $current_user->roles) ) {
            $args['role__in'] = ['ecp_client', 'scp_client', 'ecp_client_manager', 'ecp_business_admin', 'administrator'];
        } elseif ( is_object($current_user) && in_array('ecp_business_admin', $current_user->roles) ) {
            $args['role__in'] = ['ecp_client', 'scp_client', 'ecp_client_manager', 'ecp_business_admin'];
        } elseif ( is_object($current_user) && in_array('ecp_client_manager', $current_user->roles) ) {
    ...
    public static function update_client_user( $data ) {
        ...
        if (isset($data['ecp_user_role']) && current_user_can('promote_users')) {
        ...
        if ( ! empty( $data['ecp_user_password'] ) ) {
            ...
            // ** FIX: If the admin changed their own password, send a special response to trigger a logout. **
            if ( $user_id === $current_user_id ) {
                return [
                    'success' => true,
                    'message' => __( 'Password updated successfully. You will now be logged out for security.', 'ecp' ),
