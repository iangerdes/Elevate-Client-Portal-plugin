 ...
 * @version 120.0.0 (Admin Permissions Fix)
 * @comment Updated the role registration to explicitly grant all custom capabilities to the Administrator role, ensuring they can see all dashboard sections.
 */
...
class ECP_User_Manager {
    ...
    public static function register_custom_roles() {
        ...
        // ** FIX: Explicitly grant all custom capabilities to the Administrator role. **
    ...
    public static function save_role_permissions_logic($data) {
        // Add a strict permission check to ensure only administrators can execute this function.
        ...
        foreach ($editable_roles as $role_slug) {
            ...
            foreach ($all_caps as $group) {
                foreach ($group as $cap_slug => $cap_name) {
                    // ** FIX: Protect the core 'read' capability for the 'ecp_client' role. **
                    if ($role_slug === 'ecp_client' && $cap_slug === 'read') {
                        $role->add_cap('read'); // Always ensure clients can read.
                        continue;
                    }
