 ...
 * @comment Re-engineered the final password reset step to use AJAX, preventing silent failures and providing clear user feedback.
 */
class ECP_Login {
    ...
    public function handle_password_reset_forms() {
        // This function is now effectively deprecated for the initial request, but still handles the final POST from the email link.
        // We will keep it for now but the form will be changed to AJAX. The new version will make this function fully legacy.
        ...
        // This function now only handles the traditional (non-AJAX) final step of the password reset.
        // This remains as a fallback but the primary method will be AJAX.
        if ('rp' === $action && 'POST' === $_SERVER['REQUEST_METHOD'] && isset($_POST['pass1']) && !wp_doing_ajax()) {
    ...
    public function render_shortcode() {
        ...
        switch ($action) {
            case 'lostpassword':
                ...
                <form name="lostpasswordform" id="ecp-ajax-lost-password-form" method="post">
                    <p>
            ...
            case 'rp':
                ...
                if ( is_wp_error($user) ) {
                    ...
                    <div id="ecp-reset-password-messages"></div>
                    <form name="resetpassform" id="ecp-ajax-reset-password-form" method="post" autocomplete="off">
