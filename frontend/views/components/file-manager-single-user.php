 ...
 * @version 125.2.0 (Messaging Feature)
 * @comment Removed the "Notify Client" checkbox and added a dedicated "Message Client" form.
 */
...
<div class="ecp-dashboard-view ecp-file-manager" id="ecp-file-manager-view-<?php echo esc_attr($user->ID); ?>">
    <?php
    $title = __('File Manager for:', 'ecp') . ' ' . esc_html($user->display_name);
    $is_main_view = false;
    ...
    <div class="columns-2">
        ...
        <div class="ecp-file-manager-forms-col">
                ...
                <form class="ecp-upload-form" method="post" enctype="multipart/form-data">
                    ...
                    <div class="ecp-encryption-section">
                        ...
                        <div class="ecp-password-fields" style="display:none;">
                            ...
                            <p><label for="ecp-encrypt-password-confirm-<?php echo esc_attr($user->ID); ?>"><?php _e('Confirm Password:', 'ecp'); ?></label><br>
                            <input type="password" id="ecp-encrypt-password-confirm-<?php echo esc_attr($user->ID); ?>" name="ecp_encrypt_password_confirm" class="widefat" autocomplete="new-password"></p>
            ...
            <div class="postbox">
                <h2 class="hndle"><span><?php _e('Message Client', 'ecp'); ?></span></h2>
                <div class="inside">
                    <form class="ecp-ajax-form" id="ecp-message-client-form-<?php echo esc_attr($user->ID); ?>" method="post">
                        <input type="hidden" name="user_id" value="<?php echo esc_attr($user->ID); ?>">
                        <input type="hidden" name="action" value="ecp_admin_dashboard_actions">
                        <input type="hidden" name="sub_action" value="message_client">
                        <div class="form-row">
                            <label for="ecp-message-subject-<?php echo esc_attr($user->ID); ?>"><?php _e('Subject', 'ecp'); ?></label>
                            <input type="text" name="subject" id="ecp-message-subject-<?php echo esc_attr($user->ID); ?>" class="widefat" required />
                        </div>
                        <div class="form-row">
                            <label for="ecp-message-body-<?php echo esc_attr($user->ID); ?>"><?php _e('Message', 'ecp'); ?></label>
                            <textarea name="message" id="ecp-message-body-<?php echo esc_attr($user->ID); ?>" rows="6" class="widefat" required></textarea>
                        </div>
                        <p class="submit">
                            <button type="submit" class="button button-primary"><?php _e('Send Message', 'ecp'); ?></button>
                        </p>
                    </form>
                </div>
            </div>
