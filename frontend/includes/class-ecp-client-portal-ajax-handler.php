...
class ECP_Client_Portal_Ajax_Handler {
    ...
    private function __construct() {
        ...
        // ** NEW: Add AJAX endpoint for non-logged-in users. **
        add_action( 'wp_ajax_nopriv_ecp_ajax_login', [ $this, 'ajax_login_handler' ] );
    }

    /**
     * ** NEW: Handles the AJAX login request. **
     */
    public function ajax_login_handler() {
        ECP_Security_Helper::verify_nonce_or_die('ajax_login');

        // First, check if the user is disabled. We must do this manually before wp_signon.
        ...
        if ( is_wp_error($user_signon) ) {
            ...
            if ( user_can( $user_signon, 'edit_users' ) ) {
